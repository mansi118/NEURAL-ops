...
export function parseProposal(ev: ProposalEvent): Proposal | null {
  ...
  return { id, kind: "unknown", title: str(c.body) ?? "Proposal", detail: str(c.detail) ?? "" };
}

/** Encode a human verdict for the `m.neop.verdict` event the harness consumes. */
export function encodeVerdict(proposalId: string, verdict: Verdict, by?: string): Record<string, unknown> {
