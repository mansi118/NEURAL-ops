...
| **Twin live get/put** — `palace_get_twin` + `palace_put_twin` over a dedicated `twins` table (addressed, not searched) | 🟡 client green (offline; marshalling pure, envelope unwrapped, gated) · **server handlers written** (Mempalace_NEOS clone: schema+twins.ts+http+access, NOT pushed — confirm-before-push, uncompiled) · ⛔ smoke gated (Convex deploy key, joint w/ A2) |
